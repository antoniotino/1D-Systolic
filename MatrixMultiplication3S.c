/*
 ...
 Version     : 1.0
 Description : Bisogna effettuare il calcolo del prodotto C = A x B con A[m][n], B[m][n] e C[m][n].
			   Le matrici A, B e C sono suddivise in blocchi di colonne e bisogna assegnare al processo Pi, con i = 0, ..., p-1,
			   i blocchi Ai, Bi e Ci, rispettivamente di dimensione m x (n/p), m x (n/p) e m x (n/p).
...
void ripristina_sottomatrici(int* risultato, int* parziale, int rowR,int colR,int *colW,int idproc, int colP){
	...
	for(i=0;i<rowR;i++)
		for(j=0;j<colP;j++)
			risultato[i*colR+j+off]=parziale[i*colP+j];




}
/*Stampa a video la matrice. Input: MATRICE matrice da stampare, ROWS righe della matrice,COLS colonne della matrice, NOME nome della matrice*/
...
void prodotto_matrici(int* A,int* B, int* C, int rowA,int colA,int rowB,int colB,int righeBfatte){
	...
	for(int i=0;i<rowA;i++){
		for(int j=0;j<colB;j++){
			for(int k=0;k<colA;k++){//colA==rowB altrimenti non si può effettuare il prodotto
...
int main(int argc, char *argv[]){
	int nproc,myid; //identificativo di MPI
	...
	/*dimensioni della sottomatrice A di un processo */
	int *colAp,*colBp;

	/* Puntatori */
	int *Aloc,*Mtemp,*Bloc,*Cloc;
	...
	if(argc < 5){
		//prendo i dati delle dimensioni di A e B
		rowA=5;
		colA=5;
		rowB=5;
		colB=5;
	...
	/* controllo che le colonne della matrice A siano uguali al numero di colonne della matrice B */
	if(colA!=rowB) {
		if(myid==0){
	...
	if(nproc>colA){
		if(myid==0){
			printf("numero di processi %d inferiore alle colonne di a %d. Impossibile distribuire il lavoro\n", nproc, colA);
			fflush(stdout);
		}
		exit(0);
	}
	...
	if(myid == 0){
		...
		/*Se stampa è settato ad 1, si effettua la stampa delle matrici */
	...
	for(i=0;i<nproc-1;i++){
		...
		if(myid-passo<0) tempid=nproc-passo;
		else tempid=myid-passo;
